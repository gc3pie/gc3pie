...
"""
    ...
    def _setup_flavor_selection_tests(self, extra_conf=''):
        # FIXME: the test does not run if this is not in the environment!
        os.environ['OS_AUTH_URL'] = 'http://example.org:5000/v2/'

        with temporary_config(
            self._const.SSH_AUTH_STANZA
            + self._const.CLOUD_AUTH_STANZA_FULL
            + self._const.RESOURCE_STANZA + extra_conf
        ) as cfg:
            resources = cfg.make_resources()
            cloud = resources['hobbes']
    ...
    def test_flavor_selection_at_init(self):
        """
        Test that the "largest" flavor is correctly identified.
        """
        cloud, flavors = self._setup_flavor_selection_tests()
    ...
    def test_flavor_selection_default(self):
        """
...
"""
    ...
    def test_flavor_selection_application_specific(self):
        """
