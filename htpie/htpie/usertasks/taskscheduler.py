...
_taskscheduler_pool = multiprocessing.Pool(processes=5)
...
class TaskScheduler(object):
    ...
    def handle_waiting_tasks(self):
        ...
        for node_name, the_classes in fsm_classes.items():
            ...
            htpie.log.info('%d %s task(s) are going to be processed'%(to_process.count(),  node_class().cls_name))
            for a_task in to_process:
                counter += 1
                _taskscheduler_pool.apply_async(statemachine._thread_exec_fsm, (fsm_class, str(a_task.id)))
        _taskscheduler_pool.close()
        _taskscheduler_pool.join()
