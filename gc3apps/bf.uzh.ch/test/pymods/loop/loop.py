...
#  sys.exit()  
  ...
  dateString = '{:04d}-{:02d}-{:02d}-{:02d}-{:02d}-{:02d}'.format(cDate.year, cDate.month, cDate.day, cTime.hour, cTime.minute, cTime.second)
...
#    groupIndices.append(np.squeeze(list(getIndex(groupBase[ixGroup], groupRestr))).tolist())
 ...
 # sys.exit()
  ...
  for ixMeta, meta in enumerate(metaIndices):
    ...
    print('Loop iteration(ixMeta)', ixMeta, ' out of ', nMetaIndices, ' ( ', (ixMeta / nMetaIndices) * 100, '% )', file = withLog)
