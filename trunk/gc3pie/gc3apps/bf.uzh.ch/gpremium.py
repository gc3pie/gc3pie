...
Read `.loop` files and execute the `forwardPremium` program accordingly.
    ...
    def setup_options(self):
        self.add_param("-b", "--initial", metavar="DIR",
                       type=existing_directory, dest="initial",
        ...
        self.add_param("-x", "--executable", metavar="PATH", dest="executable",
                       type=executable_file, default=os.path.join(
                           os.getcwd(), "forwardPremiumOut"),
                       help="Path to the `forwardPremium` executable binary"
                       "(Default: %(default)s)")
        self.add_param("-X", "--architecture", metavar="ARCH", dest="architecture",
