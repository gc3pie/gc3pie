...
#		return targetLine	
	...
	def run(self):
		print "Checking the results of your test GAMESS calculations, the output files (exam??.out) will be taken from ", TestSet.dirLog
	    #print TestS
		LFailedTestNames = []  
		LFailedTestDesc = []  
		...
		if len(LFailedTestNames) > 0:
			...
			for test, desc in zip(LFailedTestNames, LFailedTestDesc):
				print test, ": ", desc
			print "Running the remaining",NumberOfCorrectTests," tests. "
		else:
			print "Detected",NumberOfCorrectTests, "tests."
