...
* Old-style sessions are not supported any more.  (This should not be
  a problem, as they have been automatically converted to "new-style"
  since years now.  In the unlikely case you still have an old-style
  session directory on disk, just run any session command from version
  2.4 and it will convert the format automatically.)
