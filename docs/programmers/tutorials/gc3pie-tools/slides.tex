...
% Title: What does GC3Pie behind the scene ?

% * Step1: Create an execution queue form the TaskCollection
% * Step2: Take first Task in the queue and try to find a resource where to run it
% * Step3: if resource is cloud, then select image and flavor then start a new instance
% * Step4: try to connect to the started instance - if failed, try later
% * Step5: Once managed to ssh into a running instance, create execution folder
% * Step6: Copy - using scp - the input files and, if needed, the execution wrapper
% * Step7: Start remote execution and record PID
% * Step8: Repeat for all Tasks in queue
% * Step9: Once a Task execution is completed, retrieve results as specified in 'outputs'
% * Step10: if resource is cloud and if no more Tasks to be executed, terminated running instance.
% * Step11: Done
