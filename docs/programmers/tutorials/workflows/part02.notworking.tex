...
\begin{frame}[fragile]
\frametitle{A basic example: Matlab plot}
\begin{sh}
$ matlab -nodesktop -nosplash -nodisplay -r "x  = 0: .1 : 2*pi; y1 = cos(x); y2 = sin(x); figure; plot(x, y1, 'b', x, y2, 'r-.', 'LineWidth', 2); print('myplot','-dpng'); quit()"
\end{sh}%$
\begin{tabular}{ccc}
  {\includegraphics[width=0.4\linewidth]{fig/plot01.png}}
\end{tabular}
...
class MyPlotApplication(Application):
  """Plot Trigonometric Functions"""
  def __init__(self, img):
    out = "myplot.png"
    Application.__init__(
      self,
      arguments="matlab -nodesktop -nosplash -nodisplay -r \"x  = 0: .1 : 2*pi; y1 = cos(x); y2 = sin(x); figure; plot(x, y1, 'b', x, y2, 'r-.', 'LineWidth', 2); print('myplot','-dpng'); quit()\"",
      inputs=[],
      outputs=[out],
      output_dir="mplot",
      stdout="stdout.txt")
...
class MyPlotApplication~\HL{(Application)}~:
  """Plot Trigonometric Functions."""
  def __init__(self, img):
    inp = basename(img)
    out = "gray-" + inp
...
class MyPlotApplication(Application):
  ...
  def __init__(self, img):
    ...
    Application.__init__(
      self,
      ~\HL{arguments=[}~
        ~\HL{"matlab -nodesktop -nosplash -nodisplay -r \"x  = 0: .1 : 2*pi; y1 = cos(x); y2 = sin(x); figure; plot(x, y1, 'b', x, y2, 'r-.', 'LineWidth', 2); print('myplot','-dpng'); quit()\"",}~
      inputs=[img],
      outputs=[out],
      output_dir="grayscale.d",
      stdout="stdout.txt")
...
class AScript(SessionBasedScript):
  ...
  def new_tasks(self, extra):
    ...
    app = MyPlotApplication(input_file)
    return [app]
\end{python}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{exercise*}[2.B]
    ...
    \href{https://raw.githubusercontent.com/uzh/gc3pie/training-july-2016/docs/programmers/tutorials/workflows/downloads/grayscale_app.py}{\texttt{MyPlotApplication}}
    ...
    \+ \footnotesize
    {\em (You can download the code for \texttt{MyPlotApplication} and the ``Lena'' image file from
  ...
  \begin{exercise*}[2.C]
    ...
    line, an instance of \texttt{MyPlotApplication} should be run.
  \end{exercise*}
\end{frame}
