...
\begin{frame}[fragile]
  ...
  Problem:
\begin{sh}
$ python ex5a.py --foo=1 bar baz
\end{sh}%$

  \+
  How do we translate the above shell command invocation into a
  Python procedure call \\ \texttt{ex5a('bar', 'baz', foo=1)}?
...
\begin{frame}[fragile]
  \begin{exercise*}[5.C] \emph{(Homework)}

    Modify the \texttt{topblast.py} script that you've written in Exercise 5.B
