...
$ python gasset.py parameters.csv simAsset.m
~\em [\ldots]~
        NEW 19/19  (100.0%)
    RUNNING  0/19   (0.0%)
    STOPPED  0/19   (0.0%)
...
\begin{frame}
  \begin{exercise*}[6.A]
    ...
    In the \texttt{gasset.py} script from Exercise 2.C, modify the
    \texttt{GassetApp} application to move the output picture file
...
\begin{frame}
  \begin{exercise*}[6.B]
    ...
    Modify the \texttt{gasset} script so that, when a \texttt{GassetApp}
    task has terminated execution, it prints:
    \begin{itemize}
...
\begin{frame}[fragile]\small
  ...
  \+
  Suppose you tag the \texttt{GassetApp} class by giving it this name:
\begin{python}
